...
# Dataset name.
Dataset_name="AP000423.1_replicate_1"

# N3E data.
N3E_tracks={'IP_N3E_F'  : os.path.join(PWD, "WIG", "AP000423.1", "AP000423.1_Topo_Seq_1_S5_edt_N3E_F.wig"), 
            'IP_N3E_R'  : os.path.join(PWD, "WIG", "AP000423.1", "AP000423.1_Topo_Seq_1_S5_edt_N3E_R.wig"),             
           }

# Mock data.
Mock_track_path=os.path.join(PWD, "WIG", "AP000423.1", "AP000423.1_Topo_Seq_Mock_1_S7_edt_for_rev_depth.wig")

# Path to file with GCSs coordinates.
GCS_coord_path=os.path.join(PWD, "GCS_calling", "Gyrase_Cfx_1_GCSs_calling_0_01", "Gyrase_Cfx_1_GCSs_calling_0_01Gyrase_Cfx_1_raw_dsCS_AP000423.1_called.BroadPeak")

# Genomic region to plot.
Region_to_plot=[0, 155000]
...
def plot_signal(dataset_name, Wig_tracks_dict, Mock_track_smoothed, TCSs_coords, region_to_plot, path_out):
    ...
    OriA_coord_2=135100
    OriB_coord_2=129800  
    Ori_coords=[OriA_coord_1, OriB_coord_1, OriA_coord_2, OriB_coord_2]
    ...
    LSC_reg=[0, 84300]
    IRB_reg=[84300, 110600]
    SSC_reg=[110600, 128400]
    IRA_reg=[128400, 154477]
    plots[2].set_xlim(region_to_plot)
    plots[2].set_ylim([-0.03, 0.2])
    plots[2].scatter(TSCs_ar, [0]*len(TSCs_ar), s=2, marker='o', c='black', alpha=0.1, label='TCSs')
    LSC_rect=patches.Rectangle((LSC_reg[0]+1000,0.03), LSC_reg[1]-1000, 0.04, color="#A1D99B", edgecolor="black", linewidth=1)
    plots[2].add_patch(LSC_rect)
    plots[2].annotate('LSCR', xy=(LSC_reg[1]/2, 0.035), xycoords='data', size=11)
    IRB_rect=patches.Rectangle((IRB_reg[0]+1000,0.03), IRB_reg[1]-IRB_reg[0]-1000, 0.04, color="#FC9272", edgecolor="black", linewidth=1)
    plots[2].add_patch(IRB_rect)  
    plots[2].annotate('IRB', xy=((IRB_reg[0]+IRB_reg[1])/2, 0.035), xycoords='data', size=11)
    SSC_rect=patches.Rectangle((SSC_reg[0]+1000,0.03), SSC_reg[1]-SSC_reg[0]-1000, 0.04, color="#005A32", edgecolor="black", linewidth=1)
    plots[2].add_patch(SSC_rect) 
    plots[2].annotate('SSC', xy=((SSC_reg[0]+SSC_reg[1])/2, 0.035), xycoords='data', size=11)
    IRA_rect=patches.Rectangle((IRA_reg[0]+1000,0.03), IRA_reg[1]-IRA_reg[0]-1000, 0.04, color="#FC9272", edgecolor="black", linewidth=1)
    plots[2].add_patch(IRA_rect) 
    plots[2].annotate('IRA', xy=((IRA_reg[0]+IRA_reg[1])/2, 0.035), xycoords='data', size=11)
    plots[2].vlines(Ori_coords, -0.03, 0.2, color='red', linestyles="dashed", linewidth=1)
