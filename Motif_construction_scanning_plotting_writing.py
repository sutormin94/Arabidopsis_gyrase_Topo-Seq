...
path_to_res_score_files="C:\\Users\sutor\OneDrive\\ThinkPad_working\Sutor\Science\Arabidopsis_gyrase\Sequencing_results\\NUMT_masked\Consensus_sequence_construction_and_scanning\\"
    ...
    for k, v in input_dict.items():
        ...
        for line in filein:
            ...
            if line[0] not in ['GCSs_coordinate']:
                GCSs_dict[int(line[1])]=float(line[4]) # For BroadPeak format.
            else:
                continue
        filein.close()
...
def sorting_combining(GCSs_dict):
    ...
    print("Number of Cfx GCSs for combined motif construction: " + str(len(GCSs_set_for_motif)))  
    
    return GCSs_set_for_motif 


#######
# Calculates frequences of nucleotides in the source genome.
#######

def calc_background_frec(genomefam):
    
    letters_ar=['A', 'T', 'G', 'C']
    
    background_dict={}
    
    for letter in genomefam:
        
        if letter not in background_dict:
            background_dict[letter]=1
        else:
            background_dict[letter]+=1
            
    background_dict_clean={}
    
    for letter in letters_ar:
        
        if letter in background_dict:
            
            background_dict_clean[letter]=float(background_dict[letter])/len(genomefam)
        else:
            continue
        
    if len(background_dict_clean)==0:
        
        print('Check the alphabet! No nucleotide frequences were calculated!')
        
    else:
        print(f'Nucleotide frequences in the source sequence: {background_dict_clean}')
    
    return background_dict_clean
...
def motif_construction_and_analysis(GCSs_dict, genomefam, genomefas, target_name, outpath):
    ...
    # Calculate background probabilities of nucleotides in the source sequence.
    background=calc_background_frec(genomefam)
    #Frozen variables.
    win_width_l=63
    win_width_r=67
    ...
    return dict_f, dict_rc, background
...
def Wrapper_motif_construct_scan(Source_genome_path, Target_genome_path, target_name, GCSs_files_paths, outpath, outpath_top_score, wig_path_out, ds_name):
    ...
    dict_f, dict_rc, background_nuc_freq=motif_construction_and_analysis(GCSs_for_motif, Source_sequence, Target_sequence, target_name, outpath)
    ar_max=combine_score_fw_rc(dict_f, dict_rc, Target_sequence, wig_path_out, ds_name, chr_name)
    if Target_genome_path==Source_genome_path:
        top_scored_sites_and_GCSs(ar_max, GCSs_sets, outpath_top_score)
        
    return GCSs_for_motif, background_nuc_freq


Motif_defined_GSCs_dict, Background_nuc_freq=Wrapper_motif_construct_scan(Genome_seq_path, Target_seq_path, Target_seq_name, path_to_GCSs_files, Output_data_prefix, path_to_res_score_files, Output_score_wig, Dataset_name)
...
def Plotting(matrix, matrix_type, win_width, outpath):
    ...
    ax_range=[-win_width/2, win_width/2, 0.3, 0.70]
...
def Wrapper_motif_plotting_write(GCSs_form_motif_dict, Source_genome_path, background_nuc_freq, outpath):
    ...
    matrix_deg_red=gc_matrix_edit(pfms['GC'], background_nuc_freq, window_width)
    Plotting(matrix_deg_red, 'GC', window_width, outpath)
    write_motif_deg(matrix_deg_red, window_width, outpath)
    write_motif(pfms['A'], pfms['T'], pfms['G'], pfms['C'], background_nuc_freq, window_width, outpath)
    return

Wrapper_motif_plotting_write(Motif_defined_GSCs_dict, Genome_seq_path, Background_nuc_freq, Output_data_prefix)
